...
if request.function == 'manage':
    ...
    response.subtitle = T('数据库管理 (appadmin)')
    menu = True

ignore_rw = True
response.view = 'appadmin/appadmin.html'
if menu:
    response.menu = [[T('数据库'), False,
                 URL('index')]]
    if auth.has_membership(group_id=2):
        response.menu.append([T('design'), False, URL('admin', 'default', 'design',
                 args=[request.application])])
        response.menu.append([T('state'), False,
                 URL('state')])
        response.menu.append([T('cache'), False,
                 URL('ccache')])
...
def change_userinfo():
    record = m_db(m_db.user_info.username == request.args(0)).select().first()
    form = SQLFORM(m_db['user_info'],record,
                   labels={"username":"认证名","score":"综合分数",
                           "bean":"欢乐豆","workrate":"签到率",
                           "room":"宿舍"})
    if form.process().accepts(request.vars,session):
        response.flash("修改成功")
        redirect(URL('appadmin',"#userinfo"))
    elif form.errors:
        response.flash("修改未能成功")
    return  dict(form=form)

def add_userinfo():
    form=SQLFORM(m_db.user_info,labels={"username":"认证名","score":"综合分数",
                           "bean":"欢乐豆","workrate":"签到率",
                           "room":"宿舍"})
    if form.process().accepted:
        response.flash("添加成功")
    elif form.errors:
        response.flash("添加未能成功")
    return dict(form=form)

def change_user():
    record = db(db.auth_user.username == request.args(0)).select().first()
    form = SQLFORM(db['auth_user'], record,
                   labels={"username": "认证名", "email": "电子邮件地址",
                           "name": "真实姓名"})
    if form.process().accepts(request.vars, session):
        response.flash("修改成功")
        redirect(URL('appadmin',"#user"))
    elif form.errors:
        response.flash("修改未能成功")
    return dict(form=form)
...
def add_task():
    ...
    form = SQLFORM(m_db.task,
                   labels={"name":"任务名","create_date":"创建日期",
                           "end_date":"截止日期","info":"备注",
                           "people":"参与者","dopeople":"完成者","id":"任务编号",
                            "active":"任务是否有效","admindo":"是否需要班委确认"})
    if form.process().accepted:
        o_pop = []
        group_tag = ""
        if type(request.vars.people) is type(""):
            if request.vars.people[0] == '#':
                if request.vars.people == "#所有人":
                    for record in db(db.auth_user).select():
                        o_pop.append(record.name)
                else:
                    group_tag = request.vars.people[1:];
                    for person in get_users_in_group(group_tag):
                        o_pop.append(person)
        else:
            for item in request.vars.people:
                if item[0] == "#":
                    if item == "#所有人":
                        for record in db(db.auth_user).select():
                            o_pop.append(record.name)
        ...
        o_pop = list(set(o_pop))
        t_task = m_db(m_db.task.create_date == request.vars.create_date
                              and m_db.task.end_date == request.vars.end_date).select().first()
        t_task.update_record(people=o_pop)
    elif form.errors:
        pass
    return dict(form=form)

def delete_task():
    record = m_db(m_db.task.id == request.args(0))
    record.delete()
    redirect(URL("appadmin","#taskinfo"))

def change_task():
    record = m_db(m_db.task.id == request.args(0)).select().first()
    form = SQLFORM(m_db['task'],record,
                   labels={"name":"任务名","create_date":"创建日期",
                           "end_date":"截止日期","info":"备注",
                           "people":"参与者","dopeople":"完成者",
                           "active":"任务是否有效","admindo":"是否需要班委确认"})
    if form.process().accepts(request.vars,session):
...
def change_group():
    record = db(db.auth_group.id == request.args(0)).select().first()
    form = SQLFORM(db['auth_group'], record,
                   labels={"role": "用户组身份",
                           "description": "用户组描述"})
    if form.process().accepts(request.vars, session):
        pass
        redirect(URL('appadmin', "#groupinfo"))
    elif form.errors:
        pass
    return dict(form=form)
...
def change_relation():
    record = db(db.auth_membership.id == request.args(0)).select().first()
    form = SQLFORM(db.auth_membership,record,
                   labels={"id": "关系编号",
                           "user_id": "用户ID", "group_id": "用户组ID"})
    if form.process().accepts(request.vars, session):
        pass
